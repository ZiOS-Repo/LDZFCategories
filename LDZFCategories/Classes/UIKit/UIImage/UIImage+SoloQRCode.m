...
@implementation UIImage (SoloQRCode)

+ (UIImage *)solo_qrHUDImageByContent:(NSString *)content outputSize:(CGFloat)outputSize tintColor:(nullable UIColor *)tintColor logo:(nullable UIImage *)logo logoFrame:(CGRect)logoFrame isCorrectionHighLevel:(BOOL)isHighLevel
{
...
/// @param content 内容
+ (UIImage *)solo_qrImageByContent:(NSString *)content
{
    return [UIImage solo_qrHUDImageByContent:content outputSize:430 tintColor:nil logo:nil logoFrame:CGRectZero isCorrectionHighLevel:true];
}

/// 生成高清二维码
/// @param content 内容
/// @param outputSize 输出尺寸
+ (UIImage *)solo_qrHUDImageByContent:(NSString *)content outputSize:(CGFloat)outputSize
{
    return [UIImage solo_qrHUDImageByContent:content outputSize:outputSize tintColor:nil logo:nil logoFrame:CGRectZero isCorrectionHighLevel:true];
}
...
/// @param outputSize 输出尺寸
/// @param color 颜色
+ (UIImage *)solo_qrImageByContent:(NSString *)content outputSize:(CGFloat)outputSize color:(nullable UIColor *)color
{
    return [UIImage solo_qrHUDImageByContent:content outputSize:outputSize tintColor:color logo:nil logoFrame:CGRectZero isCorrectionHighLevel:true];
}

/// 生成高清二维码
/// @param content 内容
/// @param logo logo，默认放在中间位置
+ (UIImage *)solo_qrImageWithContent:(NSString *)content outputSize:(CGFloat)outputSize logo:(nullable UIImage *)logo;
{
    CGSize logoSize = logo.size;
    if (logoSize.width > outputSize || logoSize.height > outputSize) {
        UIImage *resultImage = [UIImage solo_qrHUDImageByContent:content outputSize:outputSize tintColor:nil logo:nil logoFrame:CGRectZero isCorrectionHighLevel:true];
        return resultImage;
    }
    CGRect frame = CGRectMake((logoSize.width - outputSize)/2, (logoSize.height - outputSize)/2, logoSize.width, logoSize.height);
    UIImage *resultImage = [UIImage solo_qrHUDImageByContent:content outputSize:outputSize tintColor:nil logo:logo logoFrame:frame isCorrectionHighLevel:true];
    return resultImage;
}
...
/**
 获取二维码内内容
 */
- (NSString *)solo_getQRCodeContentString
{
    if (!self) {
        return nil;
    }
