...
@implementation UIImage (SoloGenerate)
...
{
    return [UIImage solo_cornerRadiusImageWithColor:color targetSize:targetSize corners:UIRectCornerAllCorners cornerRadius:0 backgroundColor:[UIColor whiteColor]];
}
+ (UIImage *)solo_cornerRadiusImageWithColor:(UIColor *)color targetSize:(CGSize)targetSize cornerRadius:(CGFloat)cornerRadius
{
    return [UIImage solo_cornerRadiusImageWithColor:color targetSize:targetSize corners:UIRectCornerAllCorners cornerRadius:cornerRadius backgroundColor:[UIColor whiteColor]];
}
...
+ (UIImage *)solo_triangleImageWithSize:(CGSize)size color:(UIColor *)color direction:(SoloTriangleDirection)direction
{
    ...
    UIImage *myImage = [UIImage solo_squareImageWithColor:color targetSize:size];
    UIGraphicsBeginImageContext(rect.size);
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGPoint sPoints[3];//坐标点
    if (direction == SoloTriangleDirection_Down) {
        sPoints[0] =CGPointMake(0, 0);//坐标1
        sPoints[1] =CGPointMake(size.width, 0);//坐标2
        sPoints[2] =CGPointMake(size.width/2, size.height);//坐标3
    } else if (direction == SoloTriangleDirection_Up) {
    ...
    } else if (direction == SoloTriangleDirection_Left) {
        sPoints[0] =CGPointMake(size.width, 0);//坐标1
        sPoints[1] =CGPointMake(0, size.height/2);//坐标2
        sPoints[2] =CGPointMake(size.width, size.height);//坐标3
    } else if (direction == SoloTriangleDirection_Right) {
        sPoints[0] =CGPointMake(0, 0);//坐标1
        sPoints[1] =CGPointMake(0, size.height);//坐标2
...
+ (UIImage *)solo_imageWithSize:(CGSize)size drawBlock:(void (^)(CGContextRef context))drawBlock {
