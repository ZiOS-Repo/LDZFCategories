...
@implementation UIImage (SoloClip)

- (UIImage *)solo_imageClipRect:(CGRect)clipRect
{
...
- (nullable UIImage *)solo_imageByRoundCornerRadius:(CGFloat)radius corners:(UIRectCorner)corners borderWidth:(CGFloat)borderWidth borderColor:(nullable UIColor *)borderColor borderLineJoin:(CGLineJoin)borderLineJoin
{
...
- (nullable UIImage *)solo_imageByRoundCornerRadius:(CGFloat)radius borderWidth:(CGFloat)borderWidth borderColor:(nullable UIColor *)borderColor
{
    return [self solo_imageByRoundCornerRadius:radius
...
- (nullable UIImage *)solo_circleImageByBorderWidth:(CGFloat)borderWidth borderColor:(nullable UIColor *)borderColor
{
    if (self.size.width == self.size.height) {
        return [self solo_imageByRoundCornerRadius:self.size.width/2 borderWidth:borderWidth borderColor:borderColor];
    } else {
        ...
        UIImage *newImage = [self solo_imageClipRect:CGRectMake(pointX, pointY, min, min)];
        return [newImage solo_imageByRoundCornerRadius:min/2 borderWidth:borderWidth borderColor:borderColor];
    }
}

- (UIImage *)solo_resizableImage:(UIEdgeInsets)edgeInsets resizingMode:(UIImageResizingMode)resizingMode
{
...
- (UIImage *)solo_stretchableImage:(NSInteger)left top:(NSInteger)top
{
    UIImage *image = [self stretchableImageWithLeftCapWidth:left topCapHeight:top];
    return image;
}

- (UIImage *)solo_imageChangeSize:(CGSize)newSize isScale:(BOOL)isScale
{
    UIImage *newImage;
    if (isScale) {
